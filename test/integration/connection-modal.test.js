...
// The tests below require Sidekick Link to be unavailable, so we can trigger
...
describe('Hardware extension connection modal', () => {
    ...
    test('Message saying Sidekick Link is unavailable (BLE)', async () => {
        await driver.quit();
        driver = getDriver();
        ...
        findByText('Sidekick Link'); // Sidekick Link is mentioned in the error modal

        const logs = await getLogs();
        await expect(logs).toEqual([]);
    });

    test('Message saying Sidekick Link is unavailable (BT)', async () => {
        await loadUri(uri);
